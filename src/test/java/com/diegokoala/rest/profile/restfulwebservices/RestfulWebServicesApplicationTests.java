...
public class RestfulWebServicesApplicationTests extends AbstractTestNGSpringContextTests{
	...
	public void testHarness() throws Exception {
		mockMvc.perform(get("/api/v1/harness")).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.Message").value("Welcome to Harness, Your Deployment 1.8.6 was successful implemented")).andExpect(jsonPath("$.status").value("UP"));
	...
	public void testRoot() throws Exception {
		mockMvc.perform(get("/")).andExpect(status().isOk())
				.andExpect(content().contentType("text/plain;charset=UTF-8"))
				.andExpect(jsonPath("$.Message").value("Welcome to Harness, Your Deployment 1.6.1 was successful implemented")).andExpect(jsonPath("$.status").value("UP"));
